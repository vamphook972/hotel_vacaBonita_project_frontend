...
if (isset($_GET['nombre_hotel'])) {
    $nombre_hotel = urlencode($_GET['nombre_hotel']); 
    $API_URL = "http://localhost:3004/resenas/hotel/" . $nombre_hotel;
...
<body class="bg-gray-100 min-h-screen">
  ...
  <main class="max-w-4xl mx-auto px-4 py-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">
      Reseñas del Hotel: <?= isset($_GET['nombre_hotel']) ? htmlspecialchars($_GET['nombre_hotel']) : "Desconocido" ?>
    </h2>
      ...
      <div class="space-y-6">
        <?php foreach ($reseñas as $reseña): ?>
          <div class="bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition">
            <h3 class="text-lg font-bold text-indigo-700 mb-2">Usuario: <?= htmlspecialchars($reseña['usuario']) ?></h3>
            <p class="text-yellow-600 font-semibold mb-1">⭐ Calificación: <?= htmlspecialchars($reseña['numero_estrellas']) ?>/5</p>
            <?php if (!empty($reseña['comentario'])): ?>
              <p class="text-gray-700 italic mb-2">"<?= htmlspecialchars($reseña['comentario']) ?>"</p>
            <?php endif; ?>
